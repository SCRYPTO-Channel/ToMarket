...
class Tomartod:
    ...
    def start_farming(self):
        ...
        if isinstance(data, dict):  # Ensure data is a dictionary
            ...
            if end_farming is None:
                self.log(f"{red}end_at not found in response!")
                return False
            format_end_farming = (
            ...
            self.log(f"{red}Unexpected response format: {data}")
    ...
    def daily_claim(self):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{red}failed claim daily sign!")
            return False
    ...
    def get_tasks_list(self, token):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{red}failed get_tasks_lis")
            return None
    ...
    def start_task(self,token, task_id):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{red}failed start_task!")
            return False
        return res
    ...
    def check_task(self,token, task_id):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{red}failed check_task!")
            return None
        return res.json()
    ...
    def claim_task(self, task_id):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{red}failed claim_task!")
            return None
        return res.json()
    ...
    def evaluate_stars(self):
        ...
        if res.status_code != 200:
            self.log(f"{red}failed unlock levels!")
            return False
        return res.json()
    ...
    def rank(self, token):
        ...
        if res.status_code != 200:
            self.log(f"{red}failed get rank!")
            return False
    ...
    def count_tickets(self, token):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{red}failed get tickets!")
            return False
    ...
    def max_level(self, unused_stars):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{red}failed update lvl")
            return False
        return res.json()
    ...
    def connection_wallet(self, wallet):
        ...
        if res and res.status_code != 200:
            self.log(f"{red}failed connect_wallet")
    ...
    def get_combo_puzzle(self):
        url = 'https://raw.githubusercontent.com/boytegar/TomarketBOT/refs/heads/master/combo.json' #combo_puzzle
    ...
    def claim_puzzle(self, task_id):
        ...
        answer = combo
        self.log(f"{yellow}Trying claim puzzle, answer - {white}{answer}")
        data = json.dumps({"code": answer,
                           "task_id": task_id})
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{red}failed claim puzzle")
            return None
        return res.json()
    ...
    def check_airdrop(self, token):
        ...
        # print(res.text)
        if res.status_code != 200:
            self.log(f"{red}failed check_airdrop")
            return None, None
        try:
    ...
    def check_balance_now(self, token):
        ...
        # print(res.text)
        if res.status_code != 200:
            self.log(f"{red}failed check_balance_now")
            return None
        try:
    ...
    def claim_weekly(self, token):
        ...
        res = self.http(url, self.headers, data)
        if res.status_code != 200:
            self.log(f"{red}failed claim weekly airdrop")
            return None
        try:
            status = res.json().get('status')
            if status == 500:
                self.log(f"{red}Weekly airdrop has already been claimed")
                return None
    ...
    def claim_airdrop(self):
        ...
        print(res.text)
        if res.status_code != 200:
            self.log(f"{red}failed claim_airdrop")
            return None
        try:
            amount = res.json().get('data').get("amount")
            return amount
    ...
    def claim_launchpad_task(self):
        ...
        print(res.text)
        if res.status_code != 200:
            self.log(f"{red}failed claim_launchpad_task")
        try:
            result = res.json()
            print(result)
        except Exception as e:
    ...
    def invest_Toma(self, amount):
        ...
        print(res.text)
        if res.status_code != 200:
            self.log(f"{red}failed invest_Toma")
        try:
    ...
    def claim_duck(self):
        ...
        print(res.text)
        if res.status_code != 200:
            self.log(f"{red}failed claim_duck")
        try:
    ...
    def claim_pgc(self, address, market_uid):
        ...
        print(res.text)
        if res.status_code != 200:
            self.log(f"{red}failed claim pgc")
        try:
            result = res.json().get('data').get("success")
            print(result)
        except Exception as e:
    ...
    def sharetg(self):
        ...
        res = self.http(url, self.headers, "")
        if res.status_code != 200:
            self.log(f"{red}failed share tg")
            return False
        if res.json().get('status') == 0:
            return "ok"
    ...
    def tomato_to_star(self):
        ...
        res = self.http(url, self.headers, "")
        if res.status_code != 200:
            self.log(f"{red}failed change tomato_to_star")
            return False
        if res.json().get('status') == 0:
            return "ok"

    def answer(self):
        url = 'https://raw.githubusercontent.com/Shyzg/answer/refs/heads/main/answer.json' #answer
        try:
    ...
    def get_balance(self, token, acc, user_name, wallet):
        ...
        while True:
            ...
            if rank_info:
                ...
                self.log(f"{green}Rank: {white}{rank}, {current_lvl} lvl")
                self.log(f"{green}Available Stars : {white}{unused_stars}, {green}USED Stars: {white}{used_stars}")


            if self.unlock_levels and not rank_info:
                evaluate_stars = self.evaluate_stars()
                if evaluate_stars.get('status') == 500:
                    self.log(f"{green}Levels already unlock")
                if evaluate_stars.get('status') == 0:
                    self.log(f"{green}You have - {white}{evaluate_stars.get('data').get('stars')} stars")
                    time.sleep(1)
                    current_rank = self.unlocking_levels()
                    if current_rank:
                        self.log(f"{green}Levels unlock, your current_rank - {white}{current_rank}")
            else:
                self.log(f"{green}Levels already unlock")
            ...
            if self.upgrade_max_level and rank_info:
                ...
                if result_up.get('status') == 500 :
                    self.log(f"{red}Failed ugrade lvl! - {white}{result_up.get('message')}")
                if result_up.get('status') == 0 and result_up.get('data').get('isUpgrade') == True:
                    ...
                    self.log(f"{green}Lvl ugrade, your current_rank - {white}{current_rank}, {current_lvl} lvl")
                    time.sleep(1)
                    if self.share_tg_after_upgrade:
                        share_tg = self.sharetg()
                        if share_tg == "ok":
                            self.log(f"{green}success share tg. Reward: {white} 2000!")

            if self.use_free_spin:
                tickets = int(self.count_tickets(data))
                self.log(f'{green}you have {white}{tickets} tickets')
                if tickets > 0:
            ...
            if self.complete_task:
                ...
                for category, task_list in tasks.items():
                    ...
                    if isinstance(task_list, list):
                        for task in task_list:
                            ...
                            if task_id in skip_tasks:
                               continue
                            self.log(yellow + f"Completing {task_name}, id{task_id}?")
                            try:
                                ...
                                if check:
                                    ...
                                    if task_name == "TGE Step 1:":
                                        tom_ava = "Done"
                                    if status == 3:
                                        self.log(green + f'task already completed!')
                                        continue

                                    self.log(blue + f'Task not done. Start {task_name}')
                                    completion_response = self.start_task(token, task_id)
                                    time.sleep(1)

                                    self.log(yellow + f'try claim {task_name}')
                                    claim = self.claim_task(task_id)
                                    if claim and claim.get('status') == 500:
                                        self.log(red + f"Can't claim task - {white}{claim.get('message')}")
                                    if claim and claim.get('status') == 0:
                                        self.log(green + f'task done!')
                                    time.sleep(1)
                    ...
                    self.log(green + f"All possible tasks  been completed")

            if self.change_tomato_to_star:
                msg = self.tomato_to_star()
                if msg == "ok":
                    self.log(f"{green}success change_tomato_to_star!")

            if self.claim_weekly_airdrop:
                toma = self.claim_weekly(token)
                if toma:
                    self.log(f"{green}success claim: {white}{toma}")
            ...
            if puzzle_status == 0:
                ...
                if msg and msg.get("data") == {} and msg.get("status") == 0:
                    self.log(f"{green}Sucsess claim puzzle!")
                elif msg and msg.get("data").get("message") == "Must complement relation task":
                    self.log(f"{yellow}Not time to claim puzzle! {red}Complete the Tomarket Youtube quest first!")
                elif msg and msg.get("data").get("message") == "The result is incorrect":
                    self.log(f"{red}Puzzle answer is incorrect!")
                else:
                    self.log(f"{red}Failed claim puzzle!")

            if self.play_game:
                self.log(f"{white}Auto Play-Game is {green}enable!")
                play_pass = data.get("play_passes")
                self.log(f"{green}Available Tickets  : {white}{play_pass}")
                if int(play_pass) > 0:
            ...
            if amount:
                ...
                self.log(f'{green}Your amount Round One- {white}{formatted_float}')

            amount_now = self.check_balance_now(token)
            if amount_now:
                float_number = float(amount_now) - 0.01
                formatted_float = "{:.2f}".format(float_number)
                self.log(f'{green}Your amount now- {white}{formatted_float}')
    ...
    def load_data(self, file):
        ...
        if len(datas) <= 0:
            print(
                f"{red}0 account detected from {file}, fill your data in {file} first !{reset}"
            )
            sys.exit()
...
█▄▄▄▄▄▄▄█▄▄▄▄▄▄▄█▄▄▄█  █▄█ █▄▄▄█ █▄▄▄█     █▄▄▄█ █▄▄▄▄▄▄▄█
        ...
        print(Fore.YELLOW + "╔═════════════════════════════════════════════════════════════════╗")
        print(Fore.YELLOW + "║  " + Fore.GREEN + "• YouTube Channel :  https://www.youtube.com/@SCRYPTO-Channel  " + Fore.YELLOW + "║")
        print(Fore.YELLOW + "║  " + Fore.GREEN + "• Telegram Channel : https://t.me/SCRYPTO_Newss                " + Fore.YELLOW + "║")
        print(Fore.YELLOW + "║  " + Fore.GREEN + "• Join us now for more scripts !!                              " + Fore.YELLOW + "║")
        print(Fore.YELLOW + "╚═════════════════════════════════════════════════════════════════╝" + Style.RESET_ALL)




        total_accounts = f"{yellow}Total Accounts : {white}{len(datas)}"
        total_proxies = f"{blue}total proxies detected : {white}{len(proxies)}"
        use_proxy = True if len(proxies) > 0 else False
        proxy_status = f"{green}use proxy : {white}{use_proxy}"
        box_width = max(len(total_accounts), len(total_proxies), len(proxy_status)) + 4  # Add padding


        print(Fore.RED + "╔" + "═" * box_width + "╗" + Style.RESET_ALL)
        print(Fore.RED + f"║ {total_accounts:<{box_width - 1}}          ║" + Style.RESET_ALL)
        print(Fore.RED + "╚" + "═" * box_width + "╝" + Style.RESET_ALL)
            ...
            for no, data in enumerate(datas):
                ...
                if self.connect_wallet:
                    if len(wallets) != len(datas):
                        self.log(red + 'the number of wallets should be equal to the number of accounts! connect_wallet = False!')
                ...
                self.log(
                    f"{green}Account : {white}{no + 1}{green}/{white}{len(datas)}"
                )
                self.log(f"{green}Account USERNAME : {white}{user['username']}")
                token = self.get(id)
                if token is None:
